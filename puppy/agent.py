...
class LLMAgent(Agent):
    ...
    def _reflect(self, cur_date: date) -> None:
        ...
        self.logger.info(f"Searching legal clauses with query: {legal_search_query}")
        retrieved_legal_texts = self.brain.query_legal(legal_search_query, top_k=3)
        
        legal_context_prompt_section = ""
        if retrieved_legal_texts:
            legal_context = "\n\n---\n\n".join(retrieved_legal_texts)
            legal_context_prompt_section = (
                "**MANDATORY LEGAL COMPLIANCE CHECK:** Review the following legal provisions from the Financial Consumer Protection Act. "
                f"Ensure your final analysis and plan are fully compliant.\n[Legal Context]\n{legal_context}\n---\n"
            )
            self.logger.info(f"Found legal context: {legal_context[:200]}...")

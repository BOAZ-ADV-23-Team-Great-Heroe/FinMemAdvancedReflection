...
from .legal_rag import LegalVectorDB
...
class MemoryDB:
    ...
    def add_memory(self, symbol: str, date: date, text: Union[List[str], str]) -> None:
        ...
        self.universe[symbol]["index"].add_with_ids(emb, np.array(ids, dtype=np.int64))
        ...
        for i in range(len(text)):
            ...
            self.logger.info(f"[{self.db_name}] Added ID {ids[i]}: {text[i][:50]}...")
...
class BrainDB:
    def __init__(
        ...
        self.legal_memory = legal_memory
    ...
    def from_config(cls, config: Dict[str, Any]) -> "BrainDB":
        ...
        if not logger.handlers:
            ...
            log_path = os.path.join("data", "04_model_output_log", f"{general_config['trading_symbol']}_run.log")
            os.makedirs(os.path.dirname(log_path), exist_ok=True)
            handler = logging.FileHandler(log_path, mode="a")
            handler.setFormatter(formatter)
            logger.addHandler(handler)

        # 중첩된 딕셔너리 구조를 올바르게 처리합니다.
        emb_config = config.get("agent", {}).get(agent_name, {}).get("embedding", {}).get("detail", {})
        if not emb_config:
            raise KeyError(f"Embedding config for agent '{agent_name}' not found.")
        ...
        for layer_key in ["short", "mid", "long"]:
            ...
            memory_objects[layer_key] = MemoryDB(
                ...
                jump_threshold_upper=float(layer_config.get("jump_threshold_upper", 999999)) if layer_key != "long" else 999999,
                jump_threshold_lower=float(layer_config.get("jump_threshold_lower", -999999)) if layer_key != "short" else -999999,
                importance_score_initialization=get_importance_score_initialization_func(layer_config["importance_score_initialization"], layer_key),
                recency_score_initialization=R_ConstantInitialization(),
                compound_score_calculation=LinearCompoundScore(),
                importance_score_change_access_counter=LinearImportanceScoreChange(),
        ...
        legal_config = config["legal_db"]
        legal_db_instance = LegalVectorDB(
            index_path=legal_config["index_path"],
            docs_path=legal_config["docs_path"],
            emb_config=emb_config
        )
        ...
        return cls(
            ...
            long_term_memory=memory_objects["long"],
            legal_memory=legal_db_instance,
            logger=logger
        )
    
    def query_legal(self, query_text: str, top_k: int) -> List[str]:
        if not self.legal_memory:
            self.logger.warning("LegalVectorDB is not initialized. Returning empty list.")
            return []
        return self.legal_memory.query(query_text, top_k)
    ...
    def update_access_count_with_feed_back(self, symbol: str, ids: Union[List[int], int], feedback: int) -> None:
        ...
        valid_ids = [i for i in ids if i not in self.removed_ids]
        if not valid_ids: return

        for mem_layer in [self.short_term_memory, self.mid_term_memory, self.long_term_memory]:
            success_ids = mem_layer.update_access_count_with_feed_back(symbol, valid_ids, feedback)
            valid_ids = [i for i in valid_ids if i not in success_ids]
    ...
    def step(self) -> None:
        ...
        # 이 부분은 프로젝트의 구체적인 점프 로직에 따라 완성해야 합니다.
        # 제공된 코드에서는 점프 로직이 생략되어 있어 개념적인 형태로 복원합니다.
        for _ in range(2):
    ...
    @classmethod
    def load_checkpoint(cls, path: str, config: Dict[str, Any]):
        with open(os.path.join(path, "state_dict.pkl"), "rb") as f:
            state_dict = pickle.load(f)
        ...
        brain_instance.removed_ids = state_dict.get("removed_ids", [])
